...
function AddProd({ editbook = null, seteditbook }) {
  ...
  useEffect(() => {
    if (!user || !user.currentUser) {
      ...
      setTimeout(() => navigate('/login'), 100); // Delay navigation by 2 seconds
    } else {
  ...
  useEffect(() => {
    if (user.currentUser != null) {
      const isValid = editbook == null ? data.bookname && file && data.writer  && data.catogery && data.description : data.bookname && data.writer  && data.catogery && data.description
  ...
  return (
    <div>
      <ToastContainer />
      <div style={{ width: '100%', maxWidth: '600px', minHeight: '400px', margin: '0 auto' }}>
